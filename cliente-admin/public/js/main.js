...
const confirmationModalConfirmationButton = document.getElementById(
  "confirmation-button"
);
...
const averageRequestProcessingTimeElement = document.getElementById(
  "average-request-processing-time"
);
const averageRequestQueueWaitingTimeElement = document.getElementById(
...
const refresh = () => {
  ...
  fetch(`http://127.0.0.1:9000/requests`)
    ...
    .then((requests) => {
      ...
      averageRequestProcessingTimeElement.innerText = msToTime(
        totalProcessingTime / fulfilledRequestContainer.childElementCount
      );

      averageRequestQueueWaitingTimeElement.innerText = msToTime(
        totalQueueTime / fulfilledRequestContainer.childElementCount
      );
...
// Añadir tarjeta correspondiente al servidor cuya información se recibe como parámetro
window.addServerCard = (serverData) => {
  const containerId =
...
window.addRequestCard = (requestData) => {
  const containerId =
  ...
  if (requestData.status !== "enqueued") {
    ...
    processingTimeImage.after(
      document.createTextNode(
        " " +
          msToTime(
            new Date().getTime() - Date.parse(requestData.processingStartTime)
          )
      )
    );
